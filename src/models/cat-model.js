...
const listAllCats = async () => {
  const [rows] = await promisePool.query('SELECT wsk_cats.*, users.username AS owner_name FROM wsk_cats JOIN users ON wsk_cats.owner = users.user_id');
  console.log('rows', rows);
  return rows;
};

const findCatById = async (id) => {
  const [rows] = await promisePool.execute('SELECT wsk_cats.*, users.username AS owner_name FROM wsk_cats JOIN users ON wsk_cats.owner = users.user_id WHERE cat_id = ?', [id]);
  console.log('rows', rows);
  if (rows.length === 0) {
    return false;
  }
